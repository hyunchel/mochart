...
def day(day_time=None):
    ...
    url = utils.append_date_string(base_url, day_time, date_format="%Y-%m-%d", trailing_slash=True)
...
def month(day_time=None):
    ...
    beg, _ = utils.get_week_dates(day_time)
    url = utils.append_date_string(base_url, day_time, date_format="%Y-%m", trailing_slash=True)
    return utils.get_ranks(url, "section.box-rank-entry", parser)
...
def year(day_time=None):
    ...
    beg, _ = utils.get_week_dates(day_time)
    url = utils.append_date_string(base_url, day_time, date_format="%Y", trailing_slash=True)
    return utils.get_ranks(url, "section.box-rank-entry", parser)
